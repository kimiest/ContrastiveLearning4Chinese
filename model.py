...
class SimcseModel(nn.Module):
    ...
    def forward(self, input_ids):
        out = self.bert(input_ids=input_ids, output_hidden_states=True, return_dict=True)
        if self.pooling == 'cls':
...
def unsup_loss(y_pred, device, temp=0.05):
    '''无监督的损失函数
...
def sup_loss(y_pred, device, lamda=0.05):
    '''
    有监督损失函数
    '''
    similarities = F.cosine_similarity(y_pred.unsqueeze(0), y_pred.unsqueeze(1), dim=2)
    row = torch.arange(0, y_pred.shape[0], 3)  # row=[0, 3, 6, 9...]
    col = torch.arange(0, y_pred.shape[0])     # col=[0, 1, 2, ...]
    col = col[col % 3 != 0]                    # col=[1, 2, 4, 5, 7, 8]
